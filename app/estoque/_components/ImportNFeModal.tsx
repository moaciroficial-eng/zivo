...
function extractICMS(icmsGroup: Element | null): string | null {
  ...
  for (const v of variants) {
    ...
    if (!node) continue
    const pICMS = getVal(node, 'pICMS')
    const cst = getVal(node, 'CST') ?? getVal(node, 'CSOSN')
    if (pICMS) return cst ? `CST ${cst} / ${pICMS}%` : `${pICMS}%`
...
function parseNFe(xml: string): ParsedNFe | { error: string } {
  let doc: Document
  try {
    doc = new DOMParser().parseFromString(xml, 'text/xml')
  } catch {
    return { error: 'Arquivo XML inválido.' }
  }
  ...
  const detEls = Array.from(doc.getElementsByTagNameNS('*', 'det').length
    ? doc.getElementsByTagNameNS('*', 'det')
    : doc.getElementsByTagName('det'))

  if (!detEls.length) return { error: 'Nenhum item encontrado na NF-e. Verifique se o arquivo é uma NF-e válida.' }
...
export default function ImportNFeModal({
  ...
  userId: string
  onSuccess: (produtos: Produto[]) => void
  onClose: () => void
}) {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const { emitente: emit, items: rawItems } = result

      // Busca todas as marcas e tenta encontrar correspondência com o emitente
      ...
      setError('Erro ao processar o arquivo. Verifique se é uma NF-e válida.')
    } finally {
      setParsing(false)
    }
  }
    ...
    >
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-start justify-between p-6 border-b border-zinc-800 shrink-0">
          <div className="flex flex-col gap-3">
            ...
            {step === 'preview' && markupUsed && (
              <div className="flex items-center gap-2 text-sm text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3 py-2">
                <IconCheck size={14}/>
                Markup {markupUsed}× aplicado — preço de venda calculado automaticamente (custo × {markupUsed})
              </div>
            )}
          </div>
          ...
          <button
            onClick={onClose}
            className="p-2 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-lg transition cursor-pointer shrink-0 ml-4"
          >
            <IconX />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          {step === 'upload' ? (
            ...
            <div className="p-8 flex flex-col items-center gap-6">
              ...
              >
                <div className="w-16 h-16 rounded-2xl bg-zinc-800 group-hover:bg-indigo-500/10 border border-zinc-700 group-hover:border-indigo-500/30 flex items-center justify-center transition text-zinc-400 group-hover:text-indigo-300">
                  {parsing ? <IconSpinner /> : <IconUploadLarge />}
                </div>
                <div className="text-center">
                  <p className="font-semibold text-zinc-200 group-hover:text-white transition">
                    {parsing ? 'Processando arquivo...' : 'Selecionar arquivo XML'}
                  </p>
                  <p className="text-sm text-zinc-600 mt-1">Nota Fiscal Eletrônica (.xml)</p>
                </div>
              ...
              <div className="text-center text-xs text-zinc-600 space-y-1 max-w-sm">
                ...
                <p>Se o emitente tiver markup cadastrado em Configurações, o preço de venda é calculado automaticamente</p>
              </div>
            </div>
            ...
            <div className="flex flex-col">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-zinc-800/50">
                    {items.map(item => (
                      <tr
                        key={item.key}
                        className={`transition ${item.selected ? 'hover:bg-white/[0.02]' : 'opacity-35'}`}
                      >
                        ...
                        <td className="px-4 py-2.5 text-zinc-300 whitespace-nowrap">{item.qtd}</td>
              ...
              <div className="px-6 py-3 border-t border-zinc-800 text-xs text-zinc-600 space-y-1">
                <p>Dados fiscais (NCM, CFOP, ICMS, PIS, COFINS, CEST) serão importados automaticamente para cada produto.</p>
                <p>Produtos são importados com quantidade total (UN). Para ajustar tamanhos individuais, edite o produto após a importação.</p>
              </div>
            </div>
          )}
        </div>
